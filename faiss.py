...
index.add((image_features, imglist))
...
def text_match_image():  # upload face to face database
    ...
    #    pickle.dump((image_features, imglist), f)  # 理论上图片会随时更新，所以应该实时运行这段，否则只需要运行一次
    #print('生成特征向量数据库成功!!!')
    #image_dir = os.path.join(os.getcwd(), 'testimgs')
    #image_features, imglist =mynet.generate_imagedir_features_file('zzz222.jpg',image_dir)
    #with open('features1.pkl', 'wb') as f:
    #   pickle.dump((image_features, imglist), f)
    #print('生成特征向量数据库成功!!!')
    #获取输入的话, 计算最相似的图片
...
def image_classifier(file_path, token):
